...
// Register implements Algorithm 2 (Register) from the paper.
// It calls Algorithm 1 (Transaction) to produce tx^in, then encrypts the registration payload.
func Register(participant *zerocash.Participant, note *zerocash.Note, bid *big.Int, pk groth16.ProvingKey, noteSk []byte) (*RegisterResult, error) {
